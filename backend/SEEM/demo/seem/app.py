...
opt = init_distributed(opt, int(cfg.device.replace('cuda:', '')))
...
build model
'''
model = BaseModel(opt, build_model(opt)).from_pretrained(pretrained_pth).eval().to(cfg.device)
with torch.no_grad():
...
def inference(image, task, *args, **kwargs):
    with torch.autocast(device_type='cuda', dtype=torch.float32):
        if 'Video' in task:
            return interactive_infer_video(model, audio, image, task, cfg.device, *args, **kwargs)
        else:
            return interactive_infer_image(model, audio, image, task, cfg.device, *args, **kwargs)
