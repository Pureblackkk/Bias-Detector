...
if __name__ == '__main__':
    src_folder = '/home/pureblackkkk/data/Bias-Detector/backend/post-training/augmentations/system_watermark_test/waterbirds-landbird/[with chinese text] and [without ocean and lake]'
    target_folder = '/home/pureblackkkk/data/Bias-Detector/backend/post-training/augmentations/original_watermark_test/waterbirds-landbird/[with chinese text] and [without ocean and lake]'
    landbird_img_folder = '/home/pureblackkkk/data/Bias-Detector/Waterbirds/train/landbird'

    img_name_list = [f for f in os.listdir(src_folder)]

    # Create watermark
    watermarker = AddWatermark(
        image_size=256,  # Adjust based on your image size
        font_path="/home/pureblackkkk/data/Bias-Detector/backend/post-training/fonts/SourceHanSerifSC-ExtraLight.otf",  # Adjust the font path if necessary
    )

    for img_name in img_name_list:
        # Get img from landbird dataset
        img_path = os.path.join(landbird_img_folder, f'{img_name}.jpg')
        ...
        # Save the new img in target folder
        save_folder = os.path.join(target_folder, img_name)
        os.makedirs(save_folder, exist_ok=True)
        watermarked_image.save(os.path.join(save_folder, f'inpainting.jpg'))
